...
- ネイティブ入力ノード: `LTXLoadAudioUpload`、`LTXLoadImageUpload`、`LTXLoadImageBatchUpload`、`LTXBatchUploadedFrames`、`LTXLoadImages` でフォルダ＋音声ワークフローを ComfyUI 標準入力の延長で扱えます。
...
`HOWL AT THE HAIRPIN2.wav` という長いローカルサンプルを置いている場合、API smoke はそれを優先しつつ、追跡済みウィジェット名へコピーして検証します。
