...
`workflows/LTX_MultiImageUpload_Preview.json` is a tiny App mode workflow that uploads multiple still images through one input, loads them as an `IMAGE` batch with `LTXLoadImageBatchUpload`, tiles them with `LTXTileImageBatch`, and previews the tiled image with `PreviewImage`.
