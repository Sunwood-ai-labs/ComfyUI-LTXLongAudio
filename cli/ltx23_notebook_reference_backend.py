...
class MetadataBackedStateDictLoader:
    ...
    def metadata(self, path: str) -> dict[str, Any]:  # noqa: ARG002
...
class NotebookReferencedPromptEncoder:
    def __init__(
        ...
        *,
        ...
        module_ops = mods.text_gemma.module_ops_from_gemma_root(tokenizer_root)
...
def build_pipeline(
    *,
    ...
    quantization: Any,
    torch_compile: bool,
    prompt_encoder_device: Any = "match",
    prompt_streaming_prefetch_count: int | None = None,
) -> Any:
    mods = _runtime_modules()
    torch = mods.torch
    active_registry = mods.registry.DummyRegistry()

    class PromptEncoderAdapter:
        def __init__(self, inner: Any, model_device: Any) -> None:
            self._inner = inner
            self._disable_streaming = str(model_device).startswith("cpu")
            self._streaming_override = prompt_streaming_prefetch_count
        ...
        def __call__(self, prompts: list[str], **kwargs: Any) -> Any:
            ...
            elif self._streaming_override is not None:
                kwargs["streaming_prefetch_count"] = self._streaming_override
            return self._inner(prompts, **kwargs)

    class NotebookReferencedA2VidPipeline:
        def __init__(self) -> None:
            self.device = mods.pipeline_module.get_device()
            self.dtype = torch.bfloat16
            self._prompt_encoder_device = (
                self.device if prompt_encoder_device in (None, "", "match") else torch.device(str(prompt_encoder_device))
            )
            self.prompt_encoder = PromptEncoderAdapter(
                NotebookReferencedPromptEncoder(
                    ...
                    dtype=self.dtype,
                    device=self._prompt_encoder_device,
                    registry=active_registry,
                ),
                self._prompt_encoder_device,
            )
        ...
        def __call__(self, *args: Any, **kwargs: Any) -> Any:
            return mods.pipeline_module.A2VidPipelineTwoStage.__call__(self, *args, **kwargs)

    return NotebookReferencedA2VidPipeline()


__all__ = ["NotebookReferencedPromptEncoder", "build_pipeline"]
